...
majorCategories <- joinedTables %>% 
  ...
  summarize(
    ...
    "White Count" = floor(mean(Race_White_Count)),
    "Black Count" = floor(mean(Race_Black_Count)),
    "Asian Count" = floor(mean(Race_Asian_Count)),
    "Hispanic Count" = floor(mean(Race_Hispanic_Count)),
    "Other Count" = floor(mean(Race_OtherMulti_Count)),
...
**Hypotheses**:  
  ...
  $H_a$ : $p \ne p_0$ There exists a significant difference in the proportion of white people between college students and the national (population) average.  
