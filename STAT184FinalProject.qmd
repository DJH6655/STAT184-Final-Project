...
```
``` {r}
## Creates dot plot that shows how many college graduates from each major category are in poverty
...
ggplot(majorCategories) +
  ...
  theme_minimal()+
  theme(
    plot.title.position = "plot",#makes title formatted properly in qmd
    legend.position = "none"
  )
```
